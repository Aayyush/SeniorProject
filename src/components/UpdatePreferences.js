  ...
  function fetchAgeRange() {
    ...
    db.collection("Users")
      ...
      .then((doc) => {
        if (doc.exists) {
          setCurrValue([
            doc.data()["Preferences"]["AgeRange"],
            doc.data()["Preferences"]["MaxDistance"],
          ]);
        } else {
            ...
            <!-- begin profile-content -->*/}
              <Card>
                <Card.Body>
                  ...
                  {currValues ? (
                    <h4 className="mb-4">
                      Current age range set to : {currValues[0][0]} -{" "}
                      {currValues[0][1]} years old
                    </h4>
                  ) : null}
                  ...
                  {currValues ? (
                    <h4 className="mb-4">
                      Current max distance set to : {currValues[1]} km
                    </h4>
                  ) : null}
                  {error && <Alert variant="danger">{error}</Alert>}
                  <div className={classes.root}>
